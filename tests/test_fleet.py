...
def test_update_ship_statuses():
    ...
    The method should correctly update the status of each ship in the fleet based on the hits received.
    '''
    ship1 = Ship("Ship1", 3)
    fleet = Fleet("Test Fleet", ship1)
    fleet.ships[ship1.name]['hits'] = [1, 2, 3]  # Simulate 3 hits
    assert fleet.update_ship_statuses() == True  # All ships should be sunk
